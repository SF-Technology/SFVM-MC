...
class wvmStorages(wvmConnect):
    ...
    def create_storage_uuid(self, uuid):
        xml = """
                <pool type='dir'>
                    <name>%s</name>
    ...
    def create_bt_storage_pool(self, poolname):
        xml = """
                <pool type='dir'>
                    <name>%s</name>
                    <capacity unit='bytes'>0</capacity>
                    ...
                    <source></source>
                    <target>
                        <path>/app/clone/%s</path>
                    </target>
                </pool>""" % (poolname, poolname)
        self.define_storage(xml, 0)
        stg = self.get_storage(poolname)
        stg.create(0)
        stg.setAutostart(1)


    def create_image_pool(self):
        xml = """
                <pool type='dir'>
                    <name>image</name>
                    <capacity unit='bytes'>0</capacity>
        ...
        self.define_storage(xml, 0)
        stg = self.get_storage('image')
        stg.create(0)
        stg.setAutostart(1)


    def create_clone_pool(self):
        xml = """
                <pool type='dir'>
                    <name>clone</name>
                    <capacity unit='bytes'>0</capacity>
...
class wvmStorage(wvmConnect):
    ...
    def create_volume(self, name, size_gb, vol_fmt='qcow2', metadata=False):
        size = int(size_gb) * 1073741824
        ...
        xml = """
            ...
            </volume>""" % (name, size_gb, alloc, vol_fmt)
        self._createXML(xml, metadata)

    # add by siyuanguo 20170112
    def create_disk1(self, dev_name, name, size_gb, uuid, vol_fmt='qcow2', metadata=False):
        size = int(size_gb) * 1073741824
        storage_type = self.get_type()
        alloc = size
        if storage_type == 'dir':
        ...
        xml = """
            <volume>
                ...
                <target>
                    <format type='%s'/>
                    <path>/app/image/%s/%s</path>
                </target>
            </volume>""" % (name, size, alloc, vol_fmt, uuid, name)
        self._createXML(xml, metadata)
        attach_xml = """
            <disk type='file' device='disk'>
                <driver name='qemu' type='qcow2'/>
                <source file='/app/image/%s/%s'/>
                <target dev='%s' bus='virtio'/>
            </disk>""" % (uuid, name, dev_name)
        return attach_xml

        # add by siyuanguo 20170112

    def create_disk(self,  name, size_gb, uuid, vol_fmt='qcow2', metadata=False):
        size = int(size_gb) * 1073741824
        storage_type = self.get_type()
        alloc = size
        if storage_type == 'dir':
    ...
    def get_disk_xml(self,uuid,name,dev_name):
        attach_xml = """
            <disk type='file' device='disk'>
                <driver name='qemu' type='qcow2'/>
                <source file='/app/image/%s/%s'/>
    ...
    # add by siyuanguo 20170123
    def create_migrate_volume(self, name, size_gb, vol_fmt='qcow2', metadata=False):
        size = int(size_gb) * 1073741824
        storage_type = self.get_type()
        alloc = size
        if storage_type == 'dir':
        ...
        xml = """
            ...
            </volume>""" % (name, size_gb, alloc, vol_fmt, name)
        self._createXML(xml, metadata)
