...
class image_sync_area_list(base_define.Base):
...
def area_list_info():
    resp = image_sync_area_list()

    # area层级信息 - 总部
    area_ZB,area_DQ = area_list_hostpool_info()
    data_tol = len(area_ZB) + len(area_DQ)
    resp.area_ZB =area_ZB
    resp.area_DQ=area_DQ
    resp.data_num = data_tol

    return json_helper.format_api_resp(code=ErrorCode.SUCCESS, data=resp.to_json())

def area_list_hostpool_info():
    '''
        获取层级信息
        机房 - 网络区域 - 集群
    :return:
