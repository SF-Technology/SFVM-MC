...
from service.s_host.host_service import HostService
# from flask.ext.login import login_required
from flask_login import LoginManager,login_user,login_required,current_user


@login_required
def hostpool_monitor(hostpool_id):
    # 这里的时间是距当前时间多少分钟，或者是月，天
    start_time = request.values.get('start_time')
    end_time = request.values.get('end_time')

    if not hostpool_id or not start_time or not end_time:
    ...
    req_metrics = []
    req_ips = []
    all_hosts_nums, all_hosts_data = HostService().get_hosts_of_hostpool(hostpool_id)
