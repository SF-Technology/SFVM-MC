...
from model.const_define import ErrorCode, IPStatus, VMStatus, VMTypeStatus, ImageType, VMCreateSource, \
    EnvType, VsJobStatus, DataCenterTypeForVishnu, AuditType, ApiOrigin, IpLockStatus, DataCenterType, NetCardType
...
def instance_create_from_other_platform():
    '''
      外部平台虚拟机创建
    :return:
    '''
    _init_log('instanc_create_request_from_vishnu')
    ...
    hostpool_id = data_requset['hostpool_id']  # 特殊类型应用，返回集群id
    ...
    # 根据维石平台输入的环境、网络区域查找对于的物理集群
    if not req_env or not req_net_area:
        return json_helper.format_api_resp_msg_to_vishnu(req_id=taskid_vs, job_status=VsJobStatus.FAILED,
                                                         detail='empty input of req_net_area or req_env '
                                                                'when create instance')
    hostpool_query = hostpool_service.get_hostpool_info_zb(str(DataCenterTypeForVishnu.TYPE_DICT[req_env]),
                                                           req_net_area)
    ...
    # if not owner_exist:
    #     logging.error('task %s : no such user %s in db when create instance', task_id_kvm, owner)
    ...
    while not ip_lock_unused:
        ...
        if not ret_ip_lock_status:
            logging.error('创建VM：检查IP时无法获取资源锁状态'
                          'task %s : check ip resource can not get lock', task_id_kvm)
            return json_helper.format_api_resp_msg_to_vishnu(req_id=taskid_vs, job_status=VsJobStatus.FAILED,
    ...
    if vip_needed == '1':
        ip_need_nums = count + 1
    else:
        ip_need_nums = count
    try:
        ret_ips_status, ret_ips_data, ret_segment = __check_ip_resource(vm_env, dc_name, net_area_name, ip_need_nums)
    except Exception as e:
        _msg = '外部平台虚拟机创建：检查IP资源是否足够出现异常task_id_kvm %s: check ip resource exception when instance create from platform，err：%s' % (task_id_kvm, e)
        logging.error(_msg)
    ...
    logging.info('task %s : check ip resource succeed when create instance', task_id_kvm)
...
def _check_ip_resource(hostpool_id, count, env):
    '''
        判断IP资源是否足够
    :param hostpool_id:
    :param count:
    ...
    ips_list, segment_data = ip_service.get_available_ips(segments_list, count, env)
...
def __check_ip_resource(env, dc_name, net_area, count):
    ...
    '''
    # 查询指定环境、网络区域是否有所需网段
    ret_segment_datas = segment_s.get_segments_data_by_type(net_area, dc_name, env, NetCardType)
...
def __update_ip_lock_unused():
    ...
    if not ret_update_ip_lock:
        return False, '创建VM：检查IP时无法更新资源锁状态为未使用中'
    return True, '创建VM：检查IP时更新资源锁状态为未使用中成功'
...
def __update_ip_lock_used():
    ...
    ret_update_ip_lock = ip_l_s.IpLockService().update_ip_lock_info(update_ip_lock_data, where_ip_lock_data)
    if not ret_update_ip_lock:
        return False, '创建VM：检查IP时无法更新资源锁状态为使用中'
    return True, '创建VM：检查IP时更新资源锁状态为使用中成功'
